test("teste de post das linhas do banco de dados", async () => {
  const body = {
    nome: "Jest", // elemento a ser enviado para o banco de dados
    pontuacao: 42,
  };
...
test("teste bloqueio post das linhas do banco de dados", async () => {
  ...
  const response = await fetch("http://localhost:8080/post", {
    method: "post",
    body: JSON.stringify(body),
    headers: { "Content-Type": "application/json", referer: "https://curl.se" }, // o referer é determinado para que este teste seja sempre bloqueado
  });
